...
const routerExtension = (pi: ExtensionAPI) => {
	...
	const tryRestoreFallback = async (
		ctx: ExtensionContext,
	): Promise<boolean> => {
		if (lastNonRouterModel && (await tryFallbackByRef(ctx, lastNonRouterModel)))
			return true;
		try {
			const anyModel =
				(
			...
			if (
				anyModel &&
				(await tryFallbackByRef(ctx, `${anyModel.provider}/${anyModel.id}`))
			)
				return true;
		} catch {
			/* ignore */
	...
	const restoreStateFromSession = async (ctx: ExtensionContext) => {
		...
		await new Promise((resolve) =>
			setTimeout(resolve, SESSION_RESTORE_DELAY_MS),
		);
