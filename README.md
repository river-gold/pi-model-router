...
| `profiles.<name>.max` / `xhigh` / `high` / `medium` / `low` / `minimal` | Tier config: `{ "models": ["provider/model#thinking", ...], "thinking"?, "effort"?, "contextWindow"?, "maxTokens"? }`. `#thinking` suffix sets delegated reasoning per model; tier-level `thinking` (alias `effort`) is the default for models without `#` (`thinking` wins if both set). |
