...
const buildPi = () => {
	let cmd:any=null;
	return {
		registerCommand: (n:string,c:any)=>{ if(n==="router") cmd=c; },
		get:()=>cmd,
	};
};
const buildState = (over: Partial<any>={}) => ({
	currentConfig: { profiles:{ balanced:{ high:{ models:["openai/gpt"] } } }, historySize:0 } as RouterConfig,
	routerEnabled: true,
	selectedProfile: "balanced",
	lastDecision: { profile:"balanced", tier:"high", targetProvider:"openai", targetModelId:"gpt", targetLabel:"openai/gpt", reasoning:"r", thinking:"high", timestamp: Date.now() } as any,
	lastNonRouterModel: "openai/gpt",
	accumulatedCost: 0.01,
	debugEnabled: false,
	debugHistory: [{ profile:"balanced", tier:"high", targetProvider:"openai", targetModelId:"gpt", targetLabel:"openai/gpt", reasoning:"r", thinking:"high", timestamp: Date.now() } as any],
	lastConfigWarnings: [],
	failedByChain: new Map<string, Set<string>>(),
	...over,
});
const buildCtx = () => ({ ui:{ notify: vi.fn(), setStatus: vi.fn() } } as any);
const buildActions = () => ({ persistState: vi.fn(), updateStatus: vi.fn(), reloadConfig: vi.fn(), ensureValidActiveRouterProfile: vi.fn().mockResolvedValue(undefined) });

describe("commands boost2", () => {
	it("status with failures and warnings", async () => {
		const pi=buildPi(); const state=buildState({ failedByChain: new Map([["route:balanced:high", new Set(["openai/gpt"])], ["empty", new Set()]]), lastConfigWarnings:["w1"] }); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const c=pi.get(); const ctx=buildCtx();
	...
	it("status with empty failures shows none", async () => {
		const pi=buildPi(); const state=buildState({ failedByChain: new Map() }); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const c=pi.get(); await c.handler("status", buildCtx());
	});
	it("debug invalid arg", async () => {
		const pi=buildPi(); const state=buildState(); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const ctx=buildCtx(); await pi.get().handler("debug invalid", ctx);
		expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Usage"), "error");
	});
	it("debug toggle explicit", async () => {
		const pi=buildPi(); const state=buildState({ debugEnabled:false }); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const ctx=buildCtx(); await pi.get().handler("debug toggle", ctx);
		expect(state.debugEnabled).toBe(true);
	});
	it("debug without arg toggles", async () => {
		const pi=buildPi(); const state=buildState({ debugEnabled:false }); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const ctx=buildCtx(); await pi.get().handler("debug", ctx);
		expect(state.debugEnabled).toBe(true);
	});
	it("reset-failures success and alias", async () => {
		const pi=buildPi(); const state=buildState({ failedByChain: new Map([["a", new Set(["x"])]]) }); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const ctx=buildCtx(); await pi.get().handler("reset-failures", ctx);
	...
	it("reset-failures with args error", async () => {
		const pi=buildPi(); const state=buildState(); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const ctx=buildCtx(); await pi.get().handler("reset-failures extra", ctx);
		expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Usage"), "error");
	});
	...
	it("help with extra args error and unknown subcommand", async () => {
		const pi=buildPi(); const state=buildState(); const actions=buildActions(); registerCommands(pi as any, state as any, actions as any);
		const ctx=buildCtx(); await pi.get().handler("help extra", ctx);
		expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Usage"), "error");
		const ctx2=buildCtx(); await pi.get().handler("unknowncmd", ctx2);
