...
import { parseClassifierOutput, runClassifier, runClassifierWithFallbacksDetailed } from "./classifier";
...
import { thinkingToTier, resolveAvailableTier, buildRoutingDecision, decideRouting } from "./routing";
...
describe("coverage-boost parseCanonicalModelRef", () => {
	it("throws on missing slash", () => { expect(() => parseCanonicalModelRef("gpt4")).toThrow(); });
	it("throws on empty provider", () => { expect(() => parseCanonicalModelRef("/model")).toThrow(); });
	it("throws on empty modelId", () => { expect(() => parseCanonicalModelRef("prov/")).toThrow(); });
	it("throws on invalid thinking", () => { expect(() => parseCanonicalModelRef("prov/mod#invalid")).toThrow(); });
...
describe("coverage-boost formatModelRef", () => {
	it("with thinking", () => { expect(formatModelRef("a","b","high" as any)).toBe("a/b#high"); });
	it("without", () => { expect(formatModelRef("a","b")).toBe("a/b"); });
});
...
describe("coverage-boost normalizeClassifierConfig", () => {
	it("valid string", () => { expect(normalizeClassifierConfig("openai/gpt#high", [], "ctx")?.thinking).toBe("high"); });
	it("invalid string throws", () => {
		const w:string[]=[]; expect(normalizeClassifierConfig("bad-ref", w, "ctx")).toBeUndefined(); expect(w.length).toBe(1);
	});
	it("empty string returns undefined", () => { expect(normalizeClassifierConfig("   ", [], "ctx")).toBeUndefined(); });
	it("object with model and thinking warns", () => {
		const w:string[]=[]; const r = normalizeClassifierConfig({ model:"openai/gpt", thinking:"high" }, w, "ctx");
		expect(r?.model).toBe("openai/gpt"); expect(w.some(s=>s.includes("separate"))).toBe(true);
	});
	it("object missing model", () => {
		const w:string[]=[]; expect(normalizeClassifierConfig({ foo:"bar" }, w, "ctx")).toBeUndefined(); expect(w[0]).toContain("missing");
	});
	it("object invalid model", () => {
		const w:string[]=[]; expect(normalizeClassifierConfig({ model:"bad" }, w, "ctx")).toBeUndefined(); expect(w[0]).toContain("Invalid");
	});
...
describe("coverage-boost normalizeClassifierModels", () => {
	it("undefined returns undefined", () => { expect(normalizeClassifierModels(undefined, [], "c")).toBeUndefined(); });
	it("string valid", () => { expect(normalizeClassifierModels("openai/gpt", [], "c")?.length).toBe(1); });
	it("string invalid returns undefined", () => { expect(normalizeClassifierModels("bad", [], "c")).toBeUndefined(); });
	it("array mix valid/invalid", () => {
		const w:string[]=[]; const r = normalizeClassifierModels(["openai/gpt", "bad", "google/gemini"], w, "c");
	...
	it("invalid type number", () => {
		const w:string[]=[]; expect(normalizeClassifierModels(123, w, "c")).toBeUndefined(); expect(w[0]).toContain("expected string");
	});
});

describe("coverage-boost resolveEffectiveClassifier", () => {
	it("profile + global + low tier", () => {
		const p: RouterProfile = { classifierModels: [{ model:"openai/gpt" }], low:{ models:["google/gemini#low"] } };
...
describe("coverage-boost normalizeTierConfig", () => {
	it("non-object returns undefined", () => { expect(normalizeTierConfig(null, "p","high",[])).toBeUndefined(); });
	it("warns thinking field", () => {
		const w:string[]=[]; normalizeTierConfig({ models:["openai/gpt"], thinking:"high" }, "p","high",w);
		expect(w.some(s=>s.includes("thinking"))).toBe(true);
	});
	it("warns fallbacks and model", () => {
		const w:string[]=[]; normalizeTierConfig({ models:["openai/gpt"], fallbacks:["a"], model:"openai/gpt" } as any, "p","high",w);
	...
	it("missing models warns", () => {
		const w:string[]=[]; expect(normalizeTierConfig({}, "p","high",w)).toBeUndefined(); expect(w[0]).toContain("missing");
		expect(normalizeTierConfig({ models:[] }, "p","high",w)).toBeUndefined();
	});
	it("invalid entries", () => {
		const w:string[]=[]; const r = normalizeTierConfig({ models:[123, "", "bad", "openai/gpt"] }, "p","high",w);
		expect(r?.models).toEqual(["openai/gpt"]); expect(w.some(s=>s.includes("Invalid model entry"))).toBe(true);
	});
	it("no valid models returns undefined", () => {
		const w:string[]=[]; expect(normalizeTierConfig({ models:["bad1","bad2"] }, "p","high",w)).toBeUndefined();
	});
	it("with contextWindow/maxTokens/reasoning", () => {
		const w:string[]=[]; const r = normalizeTierConfig({ models:["openai/gpt#high"], contextWindow: 9999, maxTokens: 888, reasoning:true }, "p","high",w);
		expect(r?.contextWindow).toBe(9999); expect(r?.maxTokens).toBe(888); expect(r?.reasoning).toBe(true);
		expect(r?.resolvedContextWindow).toBe(9999); expect(r?.resolvedMaxTokens).toBe(888);
	});
	it("without contextWindow uses default", () => {
		const w:string[]=[]; const r = normalizeTierConfig({ models:["openai/gpt"] }, "p","high",w);
...
describe("coverage-boost normalizeConfig", () => {
	it("unknown top-level field", () => {
		const { warnings } = normalizeConfig({ unknownField:1, profiles:{} } as unknown as RouterConfig);
		expect(warnings.some(w=>w.includes("Unknown config field"))).toBe(true);
	});
	it("profile not object", () => {
		const { warnings } = normalizeConfig({ profiles: { bad:"string" } } as unknown as RouterConfig);
	...
	it("profile classifierModel deprecated", () => {
		const { warnings } = normalizeConfig({ profiles:{ p:{ high:{ models:["openai/gpt"] }, classifierModel:"openai/gpt" } } } as unknown as RouterConfig);
		expect(warnings.some(w=>w.includes("classifierModel is deprecated"))).toBe(true);
	});
	it("global classifierModel deprecated", () => {
		const { warnings } = normalizeConfig({ classifierModel:"openai/gpt", profiles:{ p:{ high:{ models:["openai/gpt"] } } } } as unknown as RouterConfig);
		expect(warnings.some(w=>w.includes("classifierModel is deprecated"))).toBe(true);
	});
	it("historySize valid and invalid", () => {
		const { config, warnings } = normalizeConfig({ historySize: 25, profiles:{ p:{ high:{ models:["openai/gpt"] } } } } as unknown as RouterConfig);
		expect(config.historySize).toBe(0); expect(warnings.some(w=>w.includes("Invalid historySize"))).toBe(true);
		const { config: c2 } = normalizeConfig({ historySize: 5, profiles:{ p:{ high:{ models:["openai/gpt"] } } } } as unknown as RouterConfig);
		expect(c2.historySize).toBe(5);
	});
	it("historyLimit alias", () => {
		const { config } = normalizeConfig({ historyLimit: 3, profiles:{ p:{ high:{ models:["openai/gpt"] } } } } as unknown as RouterConfig);
		expect(config.historySize).toBe(3);
	});
});

describe("coverage-boost profileNames/resolveProfileName", () => {
	it("sorts", () => { expect(profileNames({ profiles:{ b:{}, a:{} } } as RouterConfig)).toEqual(["a","b"]); });
	it("resolve undefined", () => { expect(resolveProfileName({ profiles:{} } as RouterConfig, "missing")).toBeUndefined(); });
	it("resolve found", () => { expect(resolveProfileName({ profiles:{ a:{} } } as RouterConfig, "a")).toBe("a"); });
});
...
describe("coverage-boost resolveContextWindow/resolveMaxTokens", () => {
	...
	it("user contextWindow precedence", () => {
		const p: RouterProfile = { high:{ models:["openai/gpt"], contextWindow: 1234, resolvedContextWindow: 9999 } };
	...
	it("registry find throws ignored", () => {
		...
		const reg = { find: () => { throw new Error("fail"); } } as any;
		expect(resolveContextWindow("high", p, reg)).toBe(7777);
	});
	it("maxTokens precedence and registry", () => {
		const p: RouterProfile = { high:{ models:["openai/gpt"], maxTokens: 111, resolvedMaxTokens: 999 } };
		...
		const regThrow = { find: () => { throw new Error("x"); } } as any;
...
describe("coverage-boost context", () => {
	...
	it("getLastUserText no user", () => { expect(getLastUserText({ messages:[] })).toBe(""); });
	it("getLastUserText with image content", () => {
		const ctx: Context = { messages:[{ role:"user", content:[{ type:"text", text:"hello" }] as any, timestamp:1 }] };
	...
	it("truncateContext single message returns as-is even if over limit", () => {
		const ctx: Context = { messages:[{ role:"user", content:"a".repeat(10000), timestamp:1 }] as unknown as Message[] };
	...
	it("truncateContext with systemPrompt and tool results orphan", () => {
		const ctx: Context = {
			...
			messages:[
				...
				{ role:"toolResult", toolCallId:"1", toolName:"t", content:"out", isError:false, timestamp:3 } as unknown as Message,
	...
	it("truncate aligns to user boundary and drops orphan toolResult", () => {
		const ctx: Context = {
			messages:[
				...
				{ role:"toolResult", toolCallId:"1", toolName:"t", content:"orphan", isError:false, timestamp:2 } as unknown as Message,
				{ role:"toolResult", toolCallId:"2", toolName:"t", content:"orphan2", isError:false, timestamp:3 } as unknown as Message,
	...
	it("estimateTokens", () => { expect(estimateTokens("abc")).toBe(1); });
});
...
describe("coverage-boost state", () => {
	it("isRouterPersistedState edge cases", () => {
		...
		expect(isRouterPersistedState({ enabled:"yes", selectedProfile:"a", timestamp:1 })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, debugHistory:"notarray" as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, accumulatedCost: -1 })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, accumulatedCost: Infinity })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, accumulatedCost: "bad" as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, lastDecision: "bad" as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, lastDecision: [] as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, lastDecision:{ profile:"p" } as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, debugEnabled: "yes" as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, lastNonRouterModel: 123 as any })).toBe(false);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1 })).toBe(true);
		expect(isRouterPersistedState({ enabled:true, selectedProfile:"a", timestamp:1, lastDecision:{ profile:"p", tier:"high", targetProvider:"a", targetModelId:"b" } as any })).toBe(true);
	});
...
describe("coverage-boost ui", () => {
	it("formatDecision with auto thinking", () => {
		const d: any = { profile:"p", tier:"low", targetProvider:"a", targetModelId:"m", targetLabel:"a/m", thinking: undefined, reasoning:"r" };
	...
	it("updateStatus mismatch profile shows waiting", () => {
		const ctx = { ui:{ setStatus: vi.fn() } } as any;
		updateStatus(ctx, true, "p", { profile:"other", tier:"high", targetProvider:"a", targetModelId:"m", thinking:"high" } as any);
...
describe("coverage-boost routing", () => {
	it("thinkingToTier off -> minimal", () => { expect(thinkingToTier("off" as any)).toBe("minimal"); });
	...
	it("buildRoutingDecision uses routed.thinking fallback", () => {
		const p: RouterProfile = { high:{ models:["openai/gpt#low"], model:"openai/gpt", thinking: undefined } };
...
describe("coverage-boost stream", () => {
	...
	it("throws when no provider", () => {
		...
		expect(() => streamDelegated(reg, { provider:"missing", id:"m" } as any, { messages:[] } as any, {} as any)).toThrow();
	});
});

describe("coverage-boost logger", () => {
	it("getLogPath", () => { expect(getLogPath()).toContain("pi-model-router.log"); });
	it("logClassifierSync does not throw", () => {
		expect(() => logClassifierSync({ timestamp: new Date().toISOString(), model:"m", fullText:"hi", success:true })).not.toThrow();
	});
});
