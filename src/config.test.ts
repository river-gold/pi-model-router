...
describe("config.ts", () => {
	...
	describe("parseConfigFile", () => {
		...
		it("should warn on invalid json", () => {
			expect(
				parseConfigFile("/path/exists-invalid-json").warnings[0],
	...
	describe("mergeConfig", () => {
		it("should merge profiles override", () => {
			...
			expect(merged.profiles.balanced.medium?.models).toEqual([
				"openai/gpt-4o-mini",
			]);
	...
	describe("normalizeTierConfig", () => {
		...
		it("should leave thinking undefined when omitted", () => {
			const w: string[] = [];
			expect(
				normalizeTierConfig({ models: ["openai/gpt-4o"] }, "p", "high", w)
					?.thinking,
			).toBeUndefined();
		});
	...
	describe("normalizeConfig", () => {
		it("should normalize", () => {
			...
			expect(
				warnings.filter(
					(w) => !w.includes("deprecated") && !w.includes('"model" is removed'),
				),
			).toEqual([]);
	...
	describe("resolveDelegatedReasoning", () => {
		it("resolves", () => {
			expect(
				resolveDelegatedReasoning(
					{ reasoning: true } as unknown as Model<Api>,
					"off",
				),
			).toBeUndefined();
			expect(
	...
	describe("resolveEffectiveClassifier", () => {
		...
		it("returns undefined when no classifier and no low tier", () => {
			...
			expect(
				resolveEffectiveClassifier(profile, undefined).classifiers,
			).toBeUndefined();
			expect(resolveEffectiveClassifier(profile, undefined).source).toBe(
				"none",
			);
		});
	});
