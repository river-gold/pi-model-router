      ...
      (async () => {
        try {
          ...
          if (isSingleTier && !isToolLoop) {
            ...
            decision = buildRoutingDecision(model.id, profile, tier, reasoning, false);
          }

          const { classifiers: effectiveClassifiers, source: classifierSource } =
            resolveEffectiveClassifier(profile, state.currentConfig.classifierModels);
          ...
          if (isSingleTier) {
            ...
            const classifierFailedSet =
              state.failedByChain.get(CLASSIFIER_CHAIN_KEY) ?? new Set<string>();
            ...
            try {
              ...
              let targetLimit: number;
              {
                ...
                if (tierForModel) {
                  ...
                  targetLimit =
                    found?.contextWindow ?? resolveContextWindow(decision.tier, profile, registry);
                  /* v8 ignore stop */
                }
              }
              /* v8 ignore next */
