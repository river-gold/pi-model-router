...
const makeRegistry = (over: any = {}) => ({
	find: vi.fn((provider: string, modelId: string) => {
		...
		if (provider === "openai" && modelId === "gpt") return { provider, id: modelId, reasoning: true } as any;
...
describe("classifier boost", () => {
	...
	it("skips failedSet", async () => {
		...
		const res = await runClassifierWithFallbacksDetailed([{ model:"openai/gpt" }], reg, ctx, 0, undefined, undefined, failed);
	...
	it("calls onAttempt and succeeds on second", async () => {
		...
		const s2 = (async function*(){ yield { type:"text_delta", delta:"Tier: high\nReasoning: ok" }; })();
		streamSimple.mockReturnValueOnce(s1 as any).mockReturnValueOnce(s2 as any);
		const onAttempt = vi.fn();
		const res = await runClassifierWithFallbacksDetailed([{ model:"openai/gpt" }, { model:"openai/gpt" }], reg, ctx, 0, undefined, onAttempt, new Set());
	...
	it("adds to failedSet on skipSession", async () => {
		...
		const res = await runClassifierWithFallbacksDetailed([{ model:"openai/bad" }], reg, ctx, 0, undefined, undefined, failed);
	...
	it("does not add to failedSet on parse failure", async () => {
		const reg = makeRegistry();
		const s = (async function*(){ yield { type:"text_delta", delta:"invalid" }; })();
	...
	it("runClassifier handles stream throw and aborted signal", async () => {
		const reg = makeRegistry();
		streamSimple.mockImplementation(() => { throw new Error("stream fail"); });
		const r = await runClassifier("openai/gpt", reg, ctx);
		expect(r).toBeUndefined();
		const controller = new AbortController(); controller.abort();
		streamSimple.mockImplementation(() => { throw new Error("aborted"); });
	...
	it("runClassifier with reasoning option", async () => {
		const reg = makeRegistry();
		const s = (async function*(){ yield { type:"text_delta", delta:"Tier: low\nReasoning: ok" }; })();
		streamSimple.mockReturnValue(s as any);
	...
	it("runClassifier with historySize >0 and history text", async () => {
		const reg = makeRegistry();
		const s = (async function*(){ yield { type:"text_delta", delta:"Tier: medium\nReasoning: ok" }; })();
		streamSimple.mockReturnValue(s as any);
