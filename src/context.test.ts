    ...
    it('should return last user text', () => {
      const ctx: Context = {
        messages: [
          ...
          { role: 'assistant', content: 'a', timestamp: 2 } as unknown as Message,
    ...
    it('should truncate oldest to fit limit', () => {
      const ctx: Context = {
        ...
        messages: [
          ...
          { role: 'user', content: 'b'.repeat(3000), timestamp: 2 } as unknown as Message,
      ...
      expect((truncated.messages[truncated.messages.length - 1] as unknown as { content: string }).content).toBe('c');
    });
