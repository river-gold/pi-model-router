...
import { isRecordablePreStreamError, normalizeFailedRef, chainKeyForRoute, CLASSIFIER_CHAIN_KEY } from './failureMemory';
...
describe('failureMemory', () => {
  ...
  it('isRecordable true for 429/5xx/auth/notfound', () => {
    expect(isRecordablePreStreamError(new Error('Routed model not found: openai/gpt-4'))).toBe(true);
    expect(isRecordablePreStreamError(new Error('No API key for routed model: openai/gpt-4'))).toBe(true);
    expect(isRecordablePreStreamError(new Error('Auth failed for routed model: openai/gpt-4: 401'))).toBe(true);
  ...
  it('isRecordable false for aborted/stale/NON_RETRYABLE', () => {
    ...
    expect(isRecordablePreStreamError(new Error('NON_RETRYABLE: Model failed after sending content.'))).toBe(false);
  });
