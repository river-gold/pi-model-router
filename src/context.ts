...
export const getHistoryPairsText = (
	context: Context,
	pairCount: number,
): string => {
	...
	const lastUserIdx =
		userIndices.length > 0 ? userIndices[userIndices.length - 1] : -1;
	const historyUserIndices =
		lastUserIdx >= 0 ? userIndices.slice(0, -1).slice(-pairCount) : [];
	...
	for (const uIdx of historyUserIndices) {
		...
		const nextUserIdx =
			pos >= 0 && pos + 1 < userIndices.length
				? userIndices[pos + 1]
				: messages.length;
		let finalText = "";
