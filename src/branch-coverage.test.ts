...
describe("branch coverage final", () => {
  ...
  it("config branches", async () => {
    ...
    resolveContextWindow(
      "high",
      { high: { models: ["openai/gpt"], resolvedContextWindow: 1000 } } as any,
      reg,
    );
    resolveMaxTokens(
      "high",
      { high: { models: ["openai/gpt"], resolvedMaxTokens: 1000 } } as any,
      reg,
    );
    // hit with no registry and with contextWindow set
    resolveContextWindow(
      "high",
      { high: { models: ["openai/gpt"], contextWindow: 123, resolvedContextWindow: 1000 } } as any,
      undefined,
    );
    resolveMaxTokens(
      "high",
      { high: { models: ["openai/gpt"], maxTokens: 123, resolvedMaxTokens: 1000 } } as any,
      undefined,
    );
  });
