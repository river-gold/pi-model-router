...
describe("index provider integration", () => {
  it("covers index getters via provider stream", async () => {
    ...
    const reg = {
      find: vi.fn(
        (p: string, id: string) =>
          ({ provider: p, id, reasoning: true, contextWindow: 5000 }) as any,
      ),
    ...
    const listeners: any[] = [];
    mockPi.on = vi.fn((e: string, h: Function) => {
      listeners.push({ e, h });
    });
    // re-init with proper listeners capturing
    const mockPi2: any = {
      registerProvider: vi.fn((n, o) => {
        mockPi2._opts = o;
      }),
      registerCommand: vi.fn(),
      setModel: vi.fn().mockResolvedValue(true),
      ...
      on: vi.fn((e: string, h: Function) => {
        if (e === "session_start") mockPi2._sessionH = h;
      }),
      getThinkingLevel: vi.fn().mockReturnValue("medium"),
    };
    ...
    providerOpts.streamSimple(
      { id: opts.models[0].id, provider: "router", contextWindow: 100000 } as any,
