...
describe("index boost2", () => {
	...
	beforeEach(() => {
		...
		mockPi={
			...
			appendEntry: vi.fn(),
			on: vi.fn((e:string,h:Function)=>{ if(!listeners[e]) listeners[e]=[]; listeners[e].push(h); }),
			getThinkingLevel: vi.fn().mockReturnValue("off"),
		};
		vi.spyOn(configModule, "loadRouterConfig").mockReturnValue({ config: { profiles:{ balanced:{ high:{ models:["openai/gpt"] } } }, debug:false } as any, warnings: [] });
	});
	...
	it("tryFallbackByRef with no slash returns false and fallback fails", async () => {
		...
		const ctx=buildCtx();
		for(const h of listeners["session_start"]||[]) await h({},ctx);
		vi.mocked(configModule.loadRouterConfig).mockReturnValue({ config: { profiles:{} } as any, warnings: [] });
		const ctx3 = buildCtx({
			model:{ provider:"router", id:"balanced" },
			modelRegistry:{ find: vi.fn(()=>undefined), list: ()=>[], getApiKeyAndHeaders: async()=>({ ok:true, apiKey:"k" }) },
			sessionManager:{ getBranch: ()=>[ { type:"custom", customType:"router-state", data:{ enabled:true, selectedProfile:"balanced", timestamp:Date.now(), lastNonRouterModel:"badrefwithoutslash" } } ] }
		});
	...
	it("tryFallbackByRef where find throws is ignored", async () => {
		...
		const badRegCtx = buildCtx({
			modelRegistry:{
				find: vi.fn(()=>{ throw new Error("find fail"); }),
		...
		for(const h of listeners["model_select"]||[]) await h({ model:{ provider:"router", id:"unknown" } }, badRegCtx);
		expect(badRegCtx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Unknown router profile"), "error");
	});

	it("persistState dedup and appendEntry throw ignored", async () => {
		mockPi.appendEntry = vi.fn(()=>{ throw new Error("append fail"); });
		routerExtension(mockPi);
		const ctx=buildCtx();
		for(const h of listeners["session_start"]||[]) await h({},ctx);
	...
	it("reloadConfig with warnings notifies", async () => {
		vi.mocked(configModule.loadRouterConfig).mockReturnValue({ config: { profiles:{ balanced:{ high:{ models:["openai/gpt"] } } }, debug:false } as any, warnings: ["warn1","warn2"] });
		routerExtension(mockPi);
		const ctx=buildCtx();
		for(const h of listeners["session_start"]||[]) await h({},ctx);
	...
	it("model_select with contextWindow mismatch triggers setModelInternally", async () => {
		...
		const mismatchCtx = buildCtx({
			modelRegistry:{
				find: vi.fn((p:string,id:string)=> ({ provider:p, id, contextWindow:999, maxTokens:999 } as any)),
				getApiKeyAndHeaders: async()=>({ ok:true, apiKey:"k" }),
			}
		});
		// event model has different contextWindow
		for(const h of listeners["model_select"]||[]) await h({ model:{ provider:"router", id:"balanced", contextWindow:111, maxTokens:111 } }, mismatchCtx);
	...
	it("turn_end restores router model when enabled", async () => {
		...
		for(const h of listeners["turn_end"]||[]) await h({},ctx);
		expect(mockPi.setModel).toHaveBeenCalledWith(expect.objectContaining({ provider:"router", id:"balanced" }));
	});
	...
	it("model_select non-router records lastNonRouterModel", async () => {
		...
		for(const h of listeners["session_start"]||[]) await h({},ctx);
		mockPi.appendEntry.mockClear();
		for(const h of listeners["model_select"]||[]) await h({ model:{ provider:"anthropic", id:"claude" } }, ctx);
		expect(ctx.ui.setHiddenThinkingLabel).toHaveBeenCalled();
		expect(mockPi.appendEntry).toHaveBeenCalledWith("router-state", expect.objectContaining({ lastNonRouterModel:"anthropic/claude" }));
	});
	...
	it("ensureValid fallback success returns early (covers tryFallbackByRef true branch)", async () => {
		...
		const successCtx = buildCtx({
			...
			sessionManager:{ getBranch: ()=>[ { type:"custom", customType:"router-state", data:{ enabled:true, selectedProfile:"balanced", timestamp:Date.now(), lastNonRouterModel:"openai/gpt" } } ] }
		});
		mockPi.setModel.mockClear();
		...
		expect(mockPi.setModel).toHaveBeenCalled();
		expect(successCtx.ui.notify).not.toHaveBeenCalledWith(expect.stringContaining("no fallback"), expect.any(String));
	});
	...
	it("model_select unknown with fallback success covers tryRestoreFallback true", async () => {
		...
		for(const h of listeners["model_select"]||[]) await h({ model:{ provider:"router", id:"unknown" } }, fallbackCtx);
		expect(mockPi.setModel).toHaveBeenCalledWith(expect.objectContaining({ provider:"anthropic", id:"claude" }));
		expect(fallbackCtx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Unknown router profile"), "error");
		expect(fallbackCtx.ui.notify).not.toHaveBeenCalledWith(expect.stringContaining("no fallback"), expect.any(String));
	});
});
