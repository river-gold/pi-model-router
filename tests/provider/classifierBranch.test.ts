...
const makeState = (over: any = {}) => ({
  currentConfig: { classifierModels: [{ model: "openai/gpt" }], historySize: 0, ...over.currentConfig },
...
describe("runClassifierBranch", () => {
  ...
  it("throws when no effectiveClassifiers", async () => {
    vi.mocked(resolveEffectiveClassifier).mockReturnValue({ classifiers: undefined, source: "none" } as any);
    await expect(
      runClassifierBranch(mockRegistry, baseProfile, makeState() as any, ctx, undefined, 0, new Set(), "src"),
    ).rejects.toThrow("No classifier available");
  });

  it("throws when signal aborted", async () => {
    vi.mocked(resolveEffectiveClassifier).mockReturnValue({ classifiers: [{ model: "openai/gpt" }], source: "global" } as any);
    const signal = { aborted: true } as any;
    await expect(
      runClassifierBranch(mockRegistry, baseProfile, makeState() as any, ctx, signal, 0, new Set(), "src"),
    ).rejects.toThrow("aborted");
  });

  it("success with result and persists failedSet", async () => {
    vi.mocked(resolveEffectiveClassifier).mockReturnValue({ classifiers: [{ model: "openai/gpt", source: "global" }], source: "global" } as any);
    const failedSet = new Set<string>(["x"]);
    vi.mocked(runClassifierWithFallbacksDetailed).mockImplementation(async (_a: any, _b: any, _c: any, _d: any, _e: any, onAttempt: any, _f: any) => {
    ...
    const res = await runClassifierBranch(mockRegistry, baseProfile, state, ctx, undefined, 0, failedSet, "src");
    expect(res.result?.tier).toBe("high");
    expect(state.failedByChain.get(CLASSIFIER_CHAIN_KEY)).toBe(failedSet);
    expect(ui.setWorkingMessage).toHaveBeenCalledWith(expect.stringContaining("Classifying via global"));
    expect(ui.setWorkingMessage).toHaveBeenCalledWith(undefined);
  });

  it("success with entry.source fallback and no thinking", async () => {
    vi.mocked(resolveEffectiveClassifier).mockReturnValue({ classifiers: [{ model: "openai/gpt" }], source: "global" } as any);
    vi.mocked(runClassifierWithFallbacksDetailed).mockImplementation(async (_a: any, _b: any, _c: any, _d: any, _e: any, onAttempt: any) => {
    ...
    const ui = { setWorkingMessage: vi.fn() };
    state.lastExtensionContext = { ui } as any;
    const res = await runClassifierBranch(mockRegistry, baseProfile, state, ctx, undefined, 0, new Set(), "fallbackSrc");
  ...
  it("stale UI catch on setWorkingMessage", async () => {
    ...
    const state: any = makeState();
    state.lastExtensionContext = { ui: { setWorkingMessage: vi.fn(() => { throw new Error("stale"); }) } } as any;
    const res = await runClassifierBranch(mockRegistry, baseProfile, state, ctx, undefined, 0, new Set(), "src");
  ...
  it("throws when classifier fails with attempts", async () => {
    ...
    const state: any = makeState();
    await expect(runClassifierBranch(mockRegistry, baseProfile, state, ctx, undefined, 0, new Set(), "src")).rejects.toThrow(
      "Classifier failed",
    );
  });
  ...
  it("throws with none when attempts empty", async () => {
    vi.mocked(resolveEffectiveClassifier).mockReturnValue({ classifiers: [{ model: "openai/gpt" }], source: "global" } as any);
    vi.mocked(runClassifierWithFallbacksDetailed).mockResolvedValue({ result: undefined, attempts: [] } as any);
    const state: any = makeState();
    await expect(runClassifierBranch(mockRegistry, baseProfile, state, ctx, undefined, 0, new Set(), "src")).rejects.toThrow(
      "none",
    );
  ...
  it("does not persist empty failedSet", async () => {
    vi.mocked(resolveEffectiveClassifier).mockReturnValue({ classifiers: [{ model: "openai/gpt" }], source: "global" } as any);
    vi.mocked(runClassifierWithFallbacksDetailed).mockResolvedValue({ result: { tier: "high", reasoning: "r" }, attempts: [] } as any);
    const state: any = makeState();
    state.failedByChain = new Map();
