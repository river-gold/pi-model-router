...
describe("provider.ts", () => {
  ...
  describe("registerRouterProvider", () => {
    ...
    it("should delegate streams and accumulate cost on success", async () => {
      ...
      const _providerStream = registeredProviderOptions?.streamSimple(model, context);
    ...
    it("should auto-truncate context if target limit is smaller than reported context window", async () => {
      ...
      vi.mocked(streamSimple).mockImplementation((_model: Model<Api>, ctx: Context) => {
      ...
      expect((truncatedContextPassed as unknown as Context)?.messages.length).toBeLessThan(
    ...
    it("should fall back when auth fails for primary model", async () => {
      let authCallCount = 0;
      (
        mockState.currentModelRegistry as NonNullable<typeof mockState.currentModelRegistry>
