...
const makeRouters = (): Record<string, Router> => ({
  auto: {
...
/** 위임/강제 effort 시나리오용 라우터 (사용자 cheap 예시와 같은 구조). 정규화를 거쳐 모델 상속이 적용됨. */
const makeDelegationRouters = (): Record<string, Router> => {
...
describe("위임 models 확장을 검증함", () => {
  describe("parseDelegatedRef 파싱을 검증함", () => {
    it("router만 있으면 기본 티어(medium)로 파싱함을 검증함", () => {
      expect(parseDelegatedRef("copilot")).toEqual({ router: "copilot", tier: "medium" });
    });
    it("router#tier를 파싱함을 검증함", () => {
      expect(parseDelegatedRef("copilot#high")).toEqual({ router: "copilot", tier: "high" });
    });
    it("router#tier#effort를 파싱함을 검증함", () => {
  ...
  describe("dereferenceTier 실시간 확장을 검증함", () => {
    ...
    it("잘못된 위임 형식 및 중복 위임 시 체인 중복 방지를 검증함", () => {
      ...
      const resolved = dereferenceTier(routers, "main", "medium");
    ...
    it("위임을 대상 tier 모델로 펼침을 검증함", () => {
      const resolved = dereferenceTier(makeRouters(), "auto", "medium");
      expect(resolved?.config.models).toEqual(["github-copilot/gpt-5.6-sol#high"]);
      expect(resolved?.routerName).toBe("auto");
      expect(resolved?.tier).toBe("medium");
      expect(resolved?.chain).toEqual(["auto#medium", "copilot#high"]);
    });
    it("원본 변경이 다음 확장에 바로 반영됨을 검증함", () => {
      const routers = makeRouters();
      expect(dereferenceTier(routers, "auto", "medium")?.config.models).toEqual([
        "github-copilot/gpt-5.6-sol#high",
      ]);
      routers.copilot!.high!.models = ["github-copilot/gpt-5.6-luna#high"];
      expect(dereferenceTier(routers, "auto", "medium")?.config.models).toEqual([
        "github-copilot/gpt-5.6-luna#high",
      ]);
    });
    it("tier 없는 위임은 기본 티어(medium)로 펼침을 검증함", () => {
      const resolved = dereferenceTier(makeRouters(), "auto", "high");
      const routers = makeRouters();
      routers.auto!.high = { models: ["@copilot"] };
      const delegated = dereferenceTier(routers, "auto", "high");
      expect(delegated?.config.models).toEqual(
        dereferenceTier(routers, "copilot", "medium")?.config.models,
      );
      expect(resolved).toBeDefined();
    });
    it("tier effort 강제가 위임 결과와 모델별 #보다 우선함을 검증함", () => {
      const routers = makeDelegationRouters();
      expect(dereferenceTier(routers, "cheap", "max")?.config.models).toEqual([
        "glm/g1#max",
        "ds/d1#max",
        "openai/luna#max",
        "xai/g#max",
      ]);
      expect(dereferenceTier(routers, "cheap", "high")?.config.models).toEqual([
        "ds/d1#high",
        "glm/g1#high",
        "openai/luna#high",
        "xai/g#high",
    ...
    it("tier effort가 없으면 모델별 #와 대상 tier 강제값을 유지함을 검증함", () => {
      const routers = makeDelegationRouters();
      expect(dereferenceTier(routers, "cheap", "low")?.config.models).toEqual([
        "ds/d1#low",
        "glm/g1#low",
        "openai/luna",
        "xai/g#medium",
    ...
    it("위임 참조의 #effort가 대상 tier 강제값보다 우선함을 검증함", () => {
      const routers = makeDelegationRouters();
      routers.cheap!.low = { models: ["@deepseek#low#off", "@glm#low#max"] };
      expect(dereferenceTier(routers, "cheap", "low")?.config.models).toEqual([
        "ds/d1#off",
        "glm/g1#max",
      ]);
    ...
    it("중첩 위임을 끝까지 펼침을 검증함", () => {
      ...
      const resolved = dereferenceTier(routers, "a", "medium");
    ...
    it("순환 위임은 undefined를 반환함을 검증함", () => {
      ...
      expect(dereferenceTier(routers, "a", "medium")).toBeUndefined();
    });
    it("자기 위재는 형제 항목을 유지하고 자기 항목만 건너뜀을 검증함", () => {
      const routers: Record<string, Router> = {
        auto: { medium: { models: ["@auto#medium", "openai/gpt"] } },
      };
      expect(dereferenceTier(routers, "auto", "medium")?.config.models).toEqual(["openai/gpt"]);
    });
    it("존재하지 않는 위임 대상은 건너뜀을 검증함", () => {
      const routers: Record<string, Router> = {
        auto: { medium: { models: ["@ghost#high"] } },
      };
      expect(dereferenceTier(routers, "auto", "medium")).toBeUndefined();
    });
    it("없는 tier는 대상 router의 위쪽 가까운 tier로 펼침을 검증함", () => {
      const routers: Record<string, Router> = {
        auto: { medium: { models: ["@copilot#low"] } },
        copilot: { high: { models: ["github-copilot/gpt-5.6-sol#high"] } },
      };
      const resolved = dereferenceTier(routers, "auto", "medium");
      expect(resolved?.config.models).toEqual(["github-copilot/gpt-5.6-sol#high"]);
      expect(resolved?.chain).toEqual(["auto#medium", "copilot#high"]);
    });
    ...
    it("대상 router에 모델 tier가 없으면 undefined를 검증함", () => {
      const routers: Record<string, Router> = {
        auto: { medium: { models: ["@copilot#high"] } },
        copilot: {},
      };
    ...
    it("대상 router 자체가 없으면 undefined를 검증함", () => {
      const routers: Record<string, Router> = {
        auto: { medium: { models: ["@ghost#high"] } },
      };
      expect(dereferenceTier(routers, "auto", "medium")).toBeUndefined();
    ...
    it("확장 예산을 넘는 위임 체인은 중단함을 검증함", () => {
      ...
      const n = 300;
      for (let i = 0; i < n; i++) {
        routers[`p${i}`] = { medium: { models: [`@p${i + 1}#medium`] } };
      }
      routers[`p${n}`] = { medium: { models: ["openai/gpt-4o"] } };
      expect(dereferenceTier(routers, "p0", "medium")).toBeUndefined();
    });
    it("예산 안의 위임 체인은 끝까지 펼침을 검증함", () => {
      ...
      routers[`p${n}`] = { medium: { models: ["openai/gpt-4o"] } };
      expect(dereferenceTier(routers, "p0", "medium")?.config.models).toEqual(["openai/gpt-4o"]);
    });
  });

  describe("resolveAvailableTierLive를 검증함", () => {
    it("위임 tier도 해석 가능 tier로 셈을 검증함", () => {
      const found = resolveAvailableTierLive(makeRouters(), "auto", "medium");
      expect(found?.tier).toBe("medium");
      expect(found?.resolved.config.models).toEqual(["github-copilot/gpt-5.6-sol#high"]);
    });
    it("깨진 위임은 같은 router의 가까운 tier로 폴백함을 검증함", () => {
      const routers: Record<string, Router> = {
        auto: {
      ...
      const found = resolveAvailableTierLive(routers, "auto", "medium");
    ...
    it("모두 해석 불가면 undefined를 검증함", () => {
      ...
      expect(resolveAvailableTierLive(routers, "auto", "medium")).toBeUndefined();
    });
    it("없는 router은 undefined를 검증함", () => {
      expect(resolveAvailableTierLive(makeRouters(), "ghost", "medium")).toBeUndefined();
    });
  });

  describe("resolvableTiers를 검증함", () => {
    it("위임 포함 해석 가능 tier 목록을 검증함", () => {
      expect(resolvableTiers(makeRouters(), "auto")).toEqual(["high", "medium", "low"]);
    });
    it("깨진 위임은 제외함을 검증함", () => {
      const routers: Record<string, Router> = {
        auto: {
      ...
      expect(resolvableTiers(routers, "auto")).toEqual(["low"]);
    });
    it("없는 router은 빈 배열을 검증함", () => {
      expect(resolvableTiers(makeRouters(), "ghost")).toEqual([]);
    });
  });
  ...
  describe("normalizeConfig 통합을 검증함", () => {
    it("auto.medium 위임 models가 로드되고 실시간 확장됨을 검증함", () => {
      ...
      expect(config.routers.auto!.medium?.models).toEqual(["@copilot#high"]);
      expect(dereferenceTier(config.routers, "auto", "medium")?.config.models).toEqual([
        "github-copilot/gpt-5.6-sol#high",
      ]);
    });
    it("위임 대상 변경이 확장 결과에 바로 반영됨을 검증함", () => {
      ...
      config.routers.copilot!.high!.models = ["openai/gpt-4o"];
      expect(dereferenceTier(config.routers, "auto", "medium")?.config.models).toEqual([
        "openai/gpt-4o",
      ]);
    });
    it("제거된 ref 필드는 경고하고 tier를 비활성화함을 검증함", () => {
      ...
      expect(config.routers.auto!.medium).toBeUndefined();
      expect(config.routers.auto!.low?.models).toEqual([
        "ollama-cloud/deepseek-v4-flash:0731#low",
      ]);
    });
    it("ref만 있던 router은 건너뜀을 검증함", () => {
      const { config, warnings } = normalizeConfig({
        routers: { auto: { medium: { ref: "copilot#high" } } },
      });
      expect(config.routers.auto).toBeUndefined();
  ...
  describe("라우팅 결정을 검증함", () => {
    it("buildRoutingDecisionLive가 위임 확장 모델과 강제 effort를 사용함을 검증함", () => {
      ...
      const decision = buildRoutingDecisionLive(routers, "auto", "low", "test");
      ...
      expect(decision.effort).toBe("off");
    ...
    it("normalizeConfig 통합으로 위임 models가 로드됨을 검증함", () => {
      ...
      expect(warnings).toEqual([]);
      expect(config.routers.auto!.low?.models).toEqual(["@deepseek#low#off"]);
      expect(dereferenceTier(config.routers, "auto", "low")?.config.models).toEqual(["x/m#off"]);
    });
  });
});
